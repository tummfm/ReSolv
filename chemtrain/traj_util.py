...
def _run_to_next_printout_neighbors(apply_fn, timings, **kwargs):
    ...
    # @jit -> 220124, Causes TypeError: cannot create weak reference to 'JittedFunction' object. Is called via scan
    # thus to performance decay by commenting it.
...
## ADAPTED TO TAKE SPECIES
def trajectory_generator_init_HFE(simulator_template, energy_fn_template,
                              ref_timings=None, quantities=None, vmap_batch=10):
    """Initializes a trajectory_generator function that computes a new
    trajectory stating at the last traj_state.
    ...
    def generate_reference_trajectory(params, sim_state, species, **kwargs):
        """
        Returns a new TrajectoryState with auxilary variables.
        ...
        energy_fn = energy_fn_template(params)
        energy_fn = partial(energy_fn, species=species)
        _, apply_fn = simulator_template(energy_fn)
        run_to_printout = _run_to_next_printout_neighbors(apply_fn, timings,
                                                          **kwargs)
        ...
        ## ADAPTED DATACLASS
        traj_state = TrajectoryStateBAR(sim_state=new_sim_state,
                                     trajectory=traj,
                                     overflow=overflow,
                                     thermostat_kbt=kbt,
                                     barostat_press=barostat_press,
        ...
        ## ADAPTED TO TAKE SPECIES
        aux_trajectory = quantity_traj_HFE(traj_state, quantities, params, species,
                                       vmap_batch)
        return traj_state.replace(aux=aux_trajectory)
...
## THE SAME FUNCTION, JUST ADAPTED TO TAKE SPECIES AS AN ARGUMENT
def quantity_traj_HFE(traj_state, quantities, energy_params=None, species=None, batch_size=1):
    """Computes quantities of interest for all states in a trajectory.
    ...
    """
    if traj_state.sim_state[0].position.ndim > 2:
        if traj_state.sim_state[0].position.ndim == 4:
              last_state, fixed_reference_nbrs = util.tree_get_single(
                 util.tree_combine(traj_state.sim_state))
        else:
              last_state, fixed_reference_nbrs = util.tree_get_single(
                 traj_state.sim_state)
    else:
    ...
    def single_state_quantities(single_snapshot):
        state, kbt = single_snapshot
        nbrs = util.neighbor_update(fixed_reference_nbrs, state)
        kwargs = {'neighbor': nbrs, 'energy_params': energy_params, 'species' : species, 'kT': kbt}
        if npt_ensemble:
            box = simulate.npt_box(state)
            kwargs['box'] = box
