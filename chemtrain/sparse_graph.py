...
def get_max_edges_max_triplets(r_cutoff, position_data, box):
    """Function takes in cutoff, position_data as np array, and box
    as np array and among all returns max edges and max triplets.

    position data should be real not fractional!
    """
    max_edges = 0
    max_triplets = 0
    ...
    for positions in position_data:
        ...
        neighbor_fn = partition.neighbor_list(
            displacement_fn, box, r_cutoff, dr_threshold=0.01,
    ...
    return max_edges, max_triplets


def get_max_edges_sum_edges(r_cutoff, position_data, box):
    """Function takes in cutoff, position_data as np array, and box
    as np array and among all returns max edges and max triplets.
    ...
    for positions in position_data:
        ...
        graph, _ = sparse_graph_from_neighborlist(displacement_fn, positions,
                                                  nbrs, r_cutoff)
        max_edges = max(max_edges, graph.n_edges)
        sum_edges += graph.n_edges
    return max_edges, sum_edges
...
def pad_forces_positions_species(species, position_data, forces_data):
    """This function pads the position_data, the forces_data, and the species_data
    depending on largest molecule or structure.
    ...
    Args:
        ...
        forces_data: A list of (N_structures,) with array of (N_particles,dim)
                     forces.

    Returns:
        species_mask: species mask as jnp.array of (N_structures, N_max_particles) with
                      padded atom types if not existent.
        all_pos: A jnp.array(N_structures, N_max_particles, dim) of padded positions.
                 Non existent positions are padded with zeros.
        all_forces: A jnp.array(N_structures, N_max_particles, dim) of padded forces.
                    Non existent forces are padded with zeros.
        """
    # canonicalize inputs to lists
    if not isinstance(position_data, list):
        n_snapshots = position_data.shape[0]
    ...
    if not isinstance(forces_data, list):
        forces_data = [forces_data[i] for i in range(n_snapshots)]

    # Compute max structures
    max_pos = max(len(i) for i in species)

    all_pos = []
    all_forces = []
    species_mask = []
    for (cur_pos, cur_force, cur_species) in zip(position_data, forces_data, species):
        pad_len = max_pos - len(cur_pos)
        dim = cur_pos.shape[1]
        cur_pos = onp.concatenate((cur_pos, onp.zeros((pad_len, dim))))
        all_pos.append(cur_pos)

        # Pad forces
        cur_force = onp.concatenate((cur_force, onp.zeros((pad_len, dim))))
        all_forces.append(cur_force)

        # species mask
        cur_species = onp.concatenate((cur_species, onp.zeros(pad_len, dtype=onp.uint8)))
        species_mask.append(cur_species)

    #  Turn list into np array(are padded, thus each list entry has same size)
    all_pos = onp.array(all_pos)
    all_forces = onp.array(all_forces)
    species_mask = onp.array(species_mask, dtype=int)
    # force_mask = all_forces.astype(dtype=bool).astype(dtype=onp.uint8)

    return all_pos, all_forces, species_mask
