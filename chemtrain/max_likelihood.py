...
def mse_loss(predictions, targets, mask=None, weight=1):
    ...
    """
    squared_differences = jnp.square(targets - predictions) * weight
...
def rmse_loss(predictions, targets, mask=None):
    """Computes mean squared error loss for given predictions and targets.
    ...
    Returns:
        Mean squared error loss value.
    """
    squared_differences = jnp.sqrt(jnp.square(targets - predictions))
    return _masked_loss(squared_differences, mask)
