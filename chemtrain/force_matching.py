...
def build_dataset_ANI1x(position_data, species_data, energy_data=None, force_data=None,
                  virial_data=None):
    """Builds the force-matching dataset depending on available data.

    Interface of force-loss function depends on dict keys set here.

    Compared to build_dataset this contains by default species.
    """
    dataset = {'R': position_data, 'species': species_data}
    if energy_data is not None:
        dataset['U'] = energy_data
    if force_data is not None:
        dataset['F'] = force_data
    if virial_data is not None:
        dataset['p'] = virial_data
    return dataset, _dataset_target_keys_ANI1x(dataset)


def build_dataset_QM7x(position_data, species_data, energy_data=None, force_data=None,
    ...
    Compared to build_dataset this contains by default species.
    """
    dataset = {'R': position_data, 'species': species_data, 'amber_energy': amber_energy_data,
               'amber_force': amber_force_data}
    if energy_data is not None:
        dataset['U'] = energy_data
    if force_data is not None:
        dataset['F'] = force_data
    if virial_data is not None:
        dataset['p'] = virial_data
    return dataset, _dataset_target_keys_QM7x(dataset)
...
def _dataset_target_keys_ANI1x(dataset):
    """Dataset keys excluding particle positions for validation loss with
    possibly masked atoms.
    """
    target_key_list = list(dataset)
    target_key_list.remove('R')
    target_key_list.remove('species')
    assert target_key_list, 'At least one target quantity needs to be supplied.'
    return target_key_list

def _dataset_target_keys_QM7x(dataset):
    """Dataset keys excluding particle positions for validation loss with
    possibly masked atoms.
    """
    target_key_list = list(dataset)
    target_key_list.remove('R')
    target_key_list.remove('species')
    target_key_list.remove('amber_energy')
    target_key_list.remove('amber_force')
    assert target_key_list, 'At least one target quantity needs to be supplied.'
    return target_key_list
...
def init_model_ANI1x(nbrs_init, energy_fn_template, virial_fn=None):
    """Initialize predictions of energy, forces and virial (if applicable)
    for a single snapshot.
    ...
    def fm_model(params, single_observation):
        positions = single_observation['R']
        species = single_observation['species']
        energy_fn = energy_fn_template(params)
        # TODO check for neighborlist overflow and hand through
        nbrs = nbrs_init.update(positions)
        energy, negative_forces = value_and_grad(energy_fn)(positions,
...
def init_model_QM7x(nbrs_init, energy_fn_template, virial_fn=None):
    """Initialize predictions of energy, forces and virial (if applicable)
    for a single snapshot.
    ...
    def fm_model(params, single_observation):
        positions = single_observation['R']
        species = single_observation['species']
        amber_energy = single_observation['amber_energy']
        amber_force = single_observation['amber_force']
        energy_fn = energy_fn_template(params)
        # TODO check for neighborlist overflow and hand through
        nbrs = nbrs_init.update(positions)
        energy, negative_forces = value_and_grad(energy_fn)(positions,
        ...
        # all_force = -negative_forces + amber_force
        # predictions = {'U': energy, 'F': all_force}
        predictions = {'U': energy, 'F': -negative_forces}
        if virial_fn is not None:
            predictions['p'] = - virial_fn(State(positions), nbrs, params)
        return predictions
...
def init_loss_fn_scaled(gamma_u=1., gamma_f=1., gamma_p=1.e-6,
                        error_fn=max_likelihood.mse_loss, individual=False,
    ...
    def loss_fn(predictions, targets, mask=None):
        ...
        if 'U' in targets.keys():  # energy loss component
            ...
            #     scale_energy = (jnp.count_nonzero(targets['F']) / 3.)**2
            # num_atoms = jnp.count_nonzero(targets['F'], axis=(1, 2)) / 3.
            # jax.debug.print("Num atoms: {num_atoms}", num_atoms=num_atoms)
            num_atoms = 1
            U_scaled_shifted = predictions['U'] * scale_U_F + shift_U_F * num_atoms
            errors['energy'] = error_fn(U_scaled_shifted, targets['U'])
            # errors['energy'] = error_fn(predictions['U'], targets['U'])
            loss_val += gamma_u * errors['energy']
        if 'F' in targets.keys():  # forces loss component
            if mask is None:  # only forces need mask, U and p are unchanged
                # mask = jnp.ones_like(predictions['F'])
                mask = jnp.where(targets['F'], 1, 0)
            # weight_force = (jnp.count_nonzero(targets['F'], axis=(1, 2)) / 3.)**2  # Scale same as in Nequip paper
            F_scaled = predictions['F'] * scale_U_F
            # jax.debug.print("scale_force: {scale_force}", scale_force=scale_force)
            # errors['forces'] = error_fn(F_scaled, targets['F'], mask, weight=weight_force[:, None, None])
            errors['forces'] = error_fn(F_scaled, targets['F'], mask)
            loss_val += gamma_f * errors['forces']
        if 'p' in targets.keys():  # virial loss component
            errors['pressure'] = error_fn(predictions['p'], targets['p'])
            loss_val += gamma_p * errors['pressure']
