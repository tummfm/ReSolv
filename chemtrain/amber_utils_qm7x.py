...
def build_amber_energy_fn(prmtop_file_path: str) -> Callable:
    ...
    if len(nbond14_pairs)!=0 and len(nonbond_pairs)!=0:
        sig_ab = np.concatenate((sig_ab,Sig_ab))
        eps_ab = np.concatenate((eps_ab,Eps_ab))
        nonbond_pairs = np.vstack((nonbond_pairs,nbond14_pairs))
