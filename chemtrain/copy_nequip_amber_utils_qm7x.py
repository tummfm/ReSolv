"""Utilities to build amber energy function.

Source: https://github.com/jax-md/jax-md/blob/84ca9a4da4ba1382027d9f64e03a9f1d11f14b49/jax_md/potentials/amber.py
"""
...
def prm_get_nonbond_terms(prm_raw_data):
    ...
    # [kcal/mol] -> [eV]
    energyConversionFactor = 0.04336
    # A -> nm
    # lengthConversionFactor = 0.1
    # Keep Å
    lengthConversionFactor = 1.0

    sigma = jnp.zeros(numTypes)
    epsilon = jnp.zeros(numTypes)
    ...
    for i in range(numTypes):
        ...
        sigma = sigma.at[i].set(sig * lengthConversionFactor)
        epsilon = epsilon.at[i].set(eps * energyConversionFactor)
...
def ener_bond(bonds, bond_types, rb0, kb0, displacement_fn):
    ...
    def compute_fn(R):
        ...
        # r (n_bond)
        r_old = jax.vmap(distance)(R1, R2)
        r_new = jax.vmap(displacement_fn)(R1, R2)
        r_new = jax.vmap(jnp.linalg.norm)(r_new)
        # jax.debug.print('r_old: ', r_old)
        # jax.debug.print('r_new: ', r_new)
        # en_val (n_bond)
        en_val = _bond_interaction(r_new)
...
def ener_angle(angles, angle_types, r_theta0, k_theta0, displacement_fn):
    ...
    def theta_fn(R1, R2, R3):
        ...
        '''
        dR21 = displacement_fn(R1, R2)
        dR23 = displacement_fn(R3, R2)
        # dR23 = R3 - R2
...
def ener_bonded(prm_raw_data, displacement_fn):
    '''
    ...
    def get_bonds_info():
        ...
        # [kcal/mol/Å^2] -> [eV / Å^2]
        forceConstConversionFactor = jnp.float32(0.04336)

        # A --> nm
        # lengthConversionFactor = jnp.float32(0.1)
        # [Å] -> [Å]
        lengthConversionFactor = jnp.float32(1.0)
    ...
    def get_dihedrals_info():
        ...
        for ph0 in prm_raw_data['DIHEDRAL_PHASE']:
            val = jnp.cos(float(ph0))
            # TODO - Use JAX compatible way
            if val < 0:
    ...
    def bond_energy_fn(R):
        ...
        return ener_bond(bonds, bond_types, r0, k0, displacement_fn)(R)

    def angle_energy_fn(R):
        # Angle
        angles, angle_types, r_theta0, k_theta0 = get_angles_info()
        return ener_angle(angles, angle_types, r_theta0, k_theta0, displacement_fn)(R)
    ...
    def compute_fn(R):
        ...
        # Comment out torsion_energy_fn as this currently causes errors with JAX
        return bond_energy_fn(R) + angle_energy_fn(R) #+ torsion_energy_fn(R)
...
def nonbonded_LJ(dr, sigma, epsilon):
    ...
    '''
    # TODO - CHECK UNITS
    # calculate_fn = energy.multiplicative_isotropic_cutoff(energy.lennard_jones, r_onset=0.8, r_cutoff=1)
    calculate_fn = energy.multiplicative_isotropic_cutoff(energy.lennard_jones, r_onset=8.0, r_cutoff=10.0)
    return calculate_fn(dr, sigma, epsilon)
    # idr = (sigma/dr)
    # idr2 = idr*idr
    # idr6 = idr2*idr2*idr2
    # idr12 = idr6*idr6
...
def ener_nonbonded14(atom_types, nonbonds, sigma, epsilon, chgs, displacement_fn):
    """
    ...
    def compute_fn(R):
        ...
        dr = jax.vmap(displacement_fn)(Rb, Ra)
        dr = jax.vmap(jnp.linalg.norm)(dr)
    ...
    return compute_fn, sig_ab, eps_ab


def ener_nonbonded_pair(atom_types, nonbonds, sigma, epsilon, chgs, displacement_fn):
    """
    ...
    at_type_a = atom_types[nonbonds[:, 0]]
    at_type_b = atom_types[nonbonds[:, 1]]
    # print("Nonbonds 0: ", nonbonds[:, 0])
    # print("Nonbonds 1: ", nonbonds[:, 1])
    sig_ab = 0.5 * (sigma[at_type_a] + sigma[at_type_b])
    eps_ab = jnp.sqrt(epsilon[at_type_a] * epsilon[at_type_b])
    #breakpoint()
    chg_a = jax_md.util.maybe_downcast(chgs[nonbonds[:, 0]])  # chg_a (n_pair)
...
# from here on it's customized code
from typing import Callable

def dummy_ener_nonbonded14():
    def fn(x):
        return 0, 0
    return fn

def dummy_ener_nonbonded_pair():
    def fn(x):
        return 0, 0
    return fn

def build_amber_energy_fn(prmtop_file_path: str, displacement_fn) -> Callable:
    """Constructs amber energy function from the given prmtop file."""
    # (1) Read a prmtop file
    prm_raw_data = amber_prmtop_load(prmtop_file_path)

    # (2) Get Bonded (Bonds/Angles/Torsions) function
    ener_bonded_fn = ener_bonded(prm_raw_data, displacement_fn)
    ...
    if len(nbond14_pairs)!=0:                                               # to avoid errors
        ener_nonbonded14_fn, Sig_ab, Eps_ab = ener_nonbonded14(atom_types, nbond14_pairs,
                                            sigma, epsilon, chgs, displacement_fn)
    else:
        ener_nonbonded14_fn = dummy_ener_nonbonded14()
    ...
    if len(nonbond_pairs)!=0:
        ener_nonbonded_fn, sig_ab, eps_ab = ener_nonbonded_pair(atom_types, nonbond_pairs,
                                                sigma, epsilon, chgs, displacement_fn)
    else:
        ener_nonbonded_fn = dummy_ener_nonbonded_pair()
    #
    # if len(nbond14_pairs)!=0 and len(nonbond_pairs)!=0:
    #     sig_ab = jnp.concatenate((sig_ab,Sig_ab))
    #     eps_ab = jnp.concatenate((eps_ab,Eps_ab))
    #     nonbond_pairs = jnp.vstack((nonbond_pairs,nbond14_pairs))
    # elif len(nbond14_pairs)!=0 and len(nonbond_pairs)==0:
    #     sig_ab = Sig_ab
    #     eps_ab = Eps_ab
    #     nonbond_pairs = nbond14_pairs



    def amber_energy_fn(atom_positions, water = False):
        # Turn fractional coordinates into Å. Box has
        # atom_positions *= unit_box_size

        en_bonded = ener_bonded_fn(atom_positions)
        en_lj, en_chg = ener_nonbonded_fn(atom_positions)
        en_lj14, en_chg14 = ener_nonbonded14_fn(atom_positions)
        if water:
            en = en_bonded + (en_chg + en_chg14)/80
        else:
            en = en_bonded + en_lj + en_lj14  #+ en_chg + en_chg14
        return en

    # if len(nonbond_pairs)!=0:
    #     result = (amber_energy_fn, nonbond_pairs, sig_ab, eps_ab)
    # else:
    #     result = [amber_energy_fn]
    return amber_energy_fn  # , nonbond_pairs, sig_ab, eps_ab
