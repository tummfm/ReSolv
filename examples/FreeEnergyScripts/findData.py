...
def get_FreeSolv_data():
    ...
    original_database_filename = '../../FreeSolv-0.51/database.pickle'
...
def get_relevant_molecules(dic_mol):
    ...
    for mol_name in dic_mol.keys():
        breakpoint()
        if (not re.search("[AaBbDdEeFfGgHhIiJjKkLlMmPpQqRrSsTtUuVvWwXxYyZz]", dic_mol[mol_name]['smiles'])) and (len(re.findall("[NnOoCc]", dic_mol[mol_name]['smiles'])) < 9):
            red_mols[mol_name] = dic_mol[mol_name]
            # Add length of moles
            red_mols[mol_name]['length'] = len(re.findall('[CcNnOo]', red_mols[mol_name]['smiles']))
    return red_mols
