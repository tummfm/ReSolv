...
if len(sys.argv) == 4:
    passed_key = int(sys.argv[2])
    model_type = str(sys.argv[3])
else:
    sys.exit('Did not passed key')
...
if __name__ == '__main__':
    ...
    if model_type == 'U_vac':
        ...
        save_checkpoint = (date + "_t_prod_" + str(t_prod_U_vac) + "ps_t_equil_" +
                           ...
                           "mem_0.97")
        best_params_path = 'TrainFreeEnergy/checkpoints/' + save_checkpoint + '_epoch499.pkl'

    if use_alanine_dipeptide:
        wandb.init(
            project='AlanineDipeptide_simulation_' + str(model_type),
            config={
    ...
    # Save
    name = 'alanine_dipeptide_' + str(passed_key) + '_' + str(model_type) + '_time_' + str(total_time_in_ps) + '_'
