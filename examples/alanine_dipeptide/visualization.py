...
def plot_histogram_density(angles, saveas, folder=''):
    ...
    annotate_alanine_histrogram()
    plt.savefig(f'output/postprocessing/{folder}histogram_density_{saveas}.pdf')
    plt.close('all')
...
def plot_1d_dihedral(angles, saveas, labels, bins=60, degrees=True,
    ...
    for i in range(n_models):
        ...
        n_traj = angles_conv.shape[0]
        h_temp = onp.zeros((n_traj, bins))
        for j in range(n_traj):
            h, x_bins = jnp.histogram(angles_conv[j, :], bins=bins,
                                      density=True)
            width = x_bins[1] - x_bins[0]
            bin_center = x_bins + width/2
            h_temp[j] = h
        ...
        plt.fill_between(bin_center[:-1], h_mean-h_std, h_mean+h_std,
    ...
    if location is not None:
        ...
        plt.legend()
    plt.savefig(f'output/postprocessing/{folder}dihedral_1D_{saveas}.pdf')
    plt.close('all')
